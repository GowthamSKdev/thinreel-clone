  ...
  return (
    <section className="Agency-section pt-60">
      <>
        <div className=" container mx-auto flex justify-center items-center flex-col text-center px-4">
          <p className="tracking-wide mb-10">
            On brief and on budget, from pitch to post.
          </p>
          <h2 className="Agency-text font-BebasNeue text-7xl sm:text-9xl md:text-[10rem] tracking-tight">
            {" "}
            <span className=" text-6xl sm:text-8xl md:text-[10rem]">
              We're all in as
            </span>{" "}
            <br />
            agency allies
          </h2>
          <p className=" font-BebasNeue text-pretty max-w-2xl w-full mt-8 tracking-wider">
            We’ve spent years inside and alongside agencies. So we know how much
            this matters. We know what clients expect. And we know the risks of
            working with the wrong production partners.
          </p>
          <p className="font-BebasNeue mt-4 text-pretty max-w-2xl w-full tracking-wider">
            That’s why we’re all in as agency allies. We’re there whether you
            need to bounce some ideas off us, or bring us in to tell a brand’s
            story. Because you can find a million production teams who just
            point and shoot from a distance. We’d rather be in the trenches with
            you.
          </p>
        </div>
      ...
      <div className="container mx-auto px-4  bg-[#5B0017] font-BebasNeue flex flex-col lg:flex-row justify-between items-center">
        <p className="text-accent text-7xl italic py-6 pb-20 lg:pb-6">
          Ready to roll?
        </p>

        <div className="flex flex-col lg:flex-row w-full lg:w-1/2 justify-between items-center text-2xl md:text-2xl xl:text-4xl">
          <p
            className="w-full border-t bordor-l-0 lg:border-l lg:border-t-0 border-dashed border-accent text-center h-full py-6 px-4 cursor-pointer"
            onClick={() => HandleScrollToSection()}
          >
            I need help on <span className="text-accent">a pitch</span>
          </p>
          <p
            className="w-full border-t bordor-l-0 lg:border-l lg:border-t-0 border-dashed border-accent text-center h-full py-6 cursor-pointer"
            onClick={() => HandleScrollToSection()}
          >
            I have a <span className="text-accent">possible project</span>
          </p>
        </div>
      </div>
    </section>
